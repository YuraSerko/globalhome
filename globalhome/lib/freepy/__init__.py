...
from lib.freepy import request
import lib.freepy.globals
from lib.freepy.globals import debug
...
class FreepyDispatcher(LineReceiver):
    ...
    def connectionMade(self):
        ...
        print "conconconconconcocnonc55555\n"
        self.conncb(self)
        print "conconconconconcon44444\n"

    def connectionLost(self, reason):
        print "conconconconcon3333\n"
        if self.discocb:
            print "conconconconconconc2222\n"
            self.discocb(reason)
        print "connectionLost in init.py!!!!\n"
        debug("connectionLost: %s" % reason)
        print "conconconconconcon\n"
    ...
    def login(self, passwd):
        ...
        print "log1i logi logi logi logi\n"
        debug(">> %s" % msg)
    ...
    def _sendCommand(self, command, bgapi):
        ...
        print "sendCommand in _init_.py !!!\n"
    ...
    def originate(self, party2dial, dest_ext_app, who_call, bgapi=True):
        print "def originate init.py !!!!\n"
        if bgapi == True:
            msg = "bgapi originate [origination_caller_id_number=%s]%s %s" % (who_call,
                                             party2dial,
                                             dest_ext_app)
            req = request.BgDialoutRequest()
        else:
