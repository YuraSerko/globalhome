...
class FsHelper(ClientFactory):
    ...
    def connect(self):
        print "def connect in fshelper 52(na4alo)\n"
        if self.freepyd:
        ...
        print "def connect in fshelper 66!!!!\n"
        reactor.connectTCP(self.host, self.port, self)
        print "def connect in fshelper 68!!!!\n"
        return self.connection_deferred
    ...
    def conncb(self, freepyd):
        ...
        self.connection_deferred = None
        print "def conncb in fshelper 75\n"
        deferred2callback.callback("Connected")
        print "def conncb in fshelper 77\n"
    ...
    def originate(self, party2dial, dest_ext_app, who_call, id_fax, bgapi=True):
        """
        ...
        def originate_inner(ignored):
            ...
            deferred = self.freepyd.originate(party2dial,
                                              ...
                                              bgapi)
            return deferred
        print "def originate in fshelper 129!!!!\n"
        d = self.connect()
        print "def originate in fshelper 131!!!!\n"
        d.addCallback(originate_inner)
        print "def originate in fshelper 133!!!!\n"
        return d
...
def test4():
    print "test4 !!!!!!\n"
    fshelper = FsHelper(host="192.168.0.251",
                        passwd="ClueCon",
                        port=8080)
    print "test4 !!!!!2222\n"
    def worked(result):
